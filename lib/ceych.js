...
class Ceych {
   ...
   * @param {number} [ttl] Overrides the default TTL of whatever was supplied to Ceych constructor or 30 seconds.
   ...
   * @param {..any} args The args that identify the target cache key.
   */
  set(funcOrOpts, updatedValue, ...args) {
    const opts = getOptions(funcOrOpts);
    const cacheKey = createCacheKey(opts.func, args, opts.suffix);
    const randomTtl = this.defaultTTL - Math.floor(Math.random() * this.defaultTTL / 2);

    if (this.stats) {
      this.stats.increment('ceych.set');
    }

    return this.cache.set(cacheKey, updatedValue, randomTtl * 1000);
  }
